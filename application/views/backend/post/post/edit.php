...
<div class="row">
  <form action="<?= base_url('post/post/edit_process'); ?>" method="post">
    <div class="col-md-8">
      <div class="grid simple ">
        ...
        <div class="grid-body no-border">
          ...
          <input class="form-control input-lg" type="text" placeholder="Judul" onkeyup="string_to_slug(this.value);" required="required" name="title" value="<?=$results['title']?>">
          <br>
          <textarea id="content_editor" name="content"><?=$results['content']?></textarea>
        </div>
      ...
      <div class="grid simple ">
        ...
        <div class="grid-body no-border">
          ...
          <div class="checkbox check-success ">
            <input id="checkbox2" type="checkbox" value="1"  name="is_publish" <?php echo ($results['is_publish'] == 1) ?  'checked="checked"' : ''?>>
          ...
          <div class="row form-row">
            ...
            <div class="col-md-12">
              <div class="form-group row">
                ...
                <div class="col-sm-8">
                  <input type="text" class="form-control" id="inputSlug" placeholder="slug" name="slug" required="required" readonly="" value="<?=$results['slug']?>">
                </div>
              </div>
              <div class="form-group row">
                <label for="inputMeta" class="col-sm-4 col-form-label">Meta Desc</label>
                <div class="col-sm-8">
                  <textarea name="meta_desc" rows="5" class="form-control" id="inputMeta"><?=$results['meta_desc']?></textarea>
                </div>
              </div>
              <div class="form-group row">
                <label for="inputImage" class="col-sm-4 col-form-label">Gambar Unggulan</label>
                <div class="col-sm-8">
                  <input type="hidden" id="imgField" name="featured_image" required="required" value="<?=$results['featured_image']?>">
                  <img src="<?= base_url('upload/')?><?=$results['featured_image']?>" id="image_preview" style="width: 100%;">
                  <br>
                  <br>
                ...
                <div class="col-sm-8">
                  <select class="select2" name="categories[]" multiple style="width: 100%">
                    <?php foreach ($categories as $val) : ?>
                      <option value="<?=$val['id']?>" <?php echo (in_array($val['id'], $results['categories'])) ?  'selected="selected"' : '' ?>><?=$val['name']?></option>
                    <?php endforeach; ?>
                  </select>
                </div>
              </div>
                ...
                <div class="col-sm-8">
                  <select class="select2" name="tags[]" multiple style="width: 100%">
                    <?php foreach ($tags as $val) : ?>
                      <option value="<?=$val['id']?>" <?php echo (in_array($val['id'], $results['tags'])) ?  'selected="selected"' : '' ?>><?=$val['name']?></option>
                    <?php endforeach; ?>
                  </select>
                </div>
              </div>
